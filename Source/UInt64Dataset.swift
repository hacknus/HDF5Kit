...
public class UInt64Dataset: Dataset {
    public subscript(slices: HyperslabIndexType...) -> [UInt64] {
    ...
    public func read(_ slices: [HyperslabIndexType]) throws -> [UInt64] {
        let filespace = space
        filespace.select(slices)
        return try read(memSpace: Dataspace(dims: filespace.selectionDims), fileSpace: filespace)
    }

    public func write(_ data: [UInt64], to slices: [HyperslabIndexType]) throws {
        let filespace = space
        filespace.select(slices)
    ...
    public func append(_ data: [UInt64], dimensions: [Int], axis: Int = 0) throws {
    ...
    public func read(memSpace: Dataspace? = nil, fileSpace: Dataspace? = nil) throws -> [UInt64] {
        let size: Int
    ...
    public func read(into pointer: UnsafeMutablePointer<UInt64>, memSpace: Dataspace? = nil, fileSpace: Dataspace? = nil) throws {
    ...
    public func write(_ data: [UInt64], memSpace: Dataspace? = nil, fileSpace: Dataspace? = nil) throws {
        let size: Int
        if let memspace = memSpace {
            size = memspace.size
        } else if let filespace = fileSpace {
    ...
    ///
    /// - precondition: The `selectionSize` of the memory Dataspace is the same as for the file Dataspace
    public func write(from pointer: UnsafePointer<UInt64>, memSpace: Dataspace? = nil, fileSpace: Dataspace? = nil) throws {
...
extension GroupType {
    /// Create a IntDataset
    public func createUInt64Dataset(_ name: String, dataspace: Dataspace) -> UInt64Dataset? {
        guard let datatype = Datatype(type: UInt64.self) else {
            return nil
        }
        let datasetID = name.withCString{ name in
            return H5Dcreate2(id, name, datatype.id, dataspace.id, 0, 0, 0)
        }
        return UInt64Dataset(id: datasetID)
    }

    /// Create a chunked IntDataset
    public func createUInt64Dataset(_ name: String, dataspace: Dataspace, chunkDimensions: [UInt64]) -> UInt64Dataset? {
        guard let datatype = Datatype(type: UInt64.self) else {
            return nil
        }
        ...
        let compLevel:UInt32 = 6
        H5Pset_deflate(plist, compLevel)
    ...
    public func createAndWriteDataset(_ name: String, dims: [Int], data: [UInt64]) throws -> UInt64Dataset {
        let space = Dataspace.init(dims: dims)
        let set = createUInt64Dataset(name, dataspace: space)!
        try set.write(data)
        return set
    }

    /// Open an existing IntDataset
    public func openUInt64Dataset(_ name: String) -> UInt64Dataset? {
        let datasetID = name.withCString{ name in
